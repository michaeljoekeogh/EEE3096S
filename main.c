...
uint8_t binaryArray[] = { 0xAA, // 10101010 in binary
		0x55, // 01010101 in binary
		0xCC, // 11001100 in binary
		0x33, // 00110011 in binary
		0xF0, // 11110000 in binary
		0x0F  // 00001111 in binary
		};
static uint8_t patterns[] = {};
volatile uint32_t delayDuration = 1;
static uint16_t binaryArrayIndex = 0;
...
int main(void) {
	...
	for (uint16_t address = 0; address < sizeof(binaryArray); address++) {
		write_to_address(address, binaryArray[address]);
	}
	...
	while (1) {
		...
		uint16_t indata = GPIOA->IDR;
	// Check if Pushbutton 0 is pressed

		if((indata & GPIO_IDR_0)==0) {
			// Button is pressed, toggle between 1-second and half-second delay

			if (delayDuration == 1) {
				delayDuration = 0.5;// change flag to allow alternation
				TIM16 -> ARR = 500;// 0.5 second delay using timer

			} else{
				delayDuration = 1; // change flag to allow alternation
				TIM16 -> ARR = 1000;// 1 second delay using timer
			}
...
// Read from EEPROM address using SPI
	...
	void set_leds(uint8_t data) {
	    // Configure GPIO pins for the LEDs as outputs
	    GPIOB->ODR = data;
	    // ... Repeat for other LEDs as needed ...
	}
...
// Timer rolled over
	void TIM16_IRQHandler(void) {
		...
		uint8_t data = read_from_address(binaryArrayIndex);
		set_leds(data);
		if (data != binaryArray[binaryArrayIndex]) {
			// SPI failure, set LEDs to 0b00000001
		set_leds(0x01);
		} else {
		...
		binaryArrayIndex++;
		if (binaryArrayIndex >= sizeof(binaryArray)) {
			binaryArrayIndex = 0; // back to the beginning
		}
	 ...
	 * @retval None
	 */
	// Function to set LEDs based on the binary data
