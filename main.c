...
int main(void)
{
  ...
  while (1)
  {
	...
	char adc_string[32];
	sprintf(adc_string, "ADC Value: %d", adc_value);
	writeLCD(adc_string);
	...
	static uint32_t last_button_press = 0;
	    ...
	    if (__HAL_GPIO_EXTI_GET_IT(Button0_Pin) != RESET)
	    {
	        ...
	        if (current_time - last_button_press >= 100) // Adjust the debounce delay as needed
	        {
	            ...
	            {
	                delay_t = 500; // Set delay to 2 Hz (500 ms)
	            }
	            else // Current delay is 2 Hz
	            {
	                delay_t = 1000; // Set delay to 1 Hz (1000 ms)
	            }
	        ...
	        // Clear the interrupt flags for Button 0
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	return (adc_val * (htim3.Init.Period + 1))/ 4096;
